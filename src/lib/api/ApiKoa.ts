...
import { as2PublicAudienceUri, objectTargets } from "../activitystreams2";
 ...
 */
export default (options: { dispatch: IDispatch<IActivityPubEvent> }): Koa => {
  const koa = new Koa()
    .use(async (ctx, next) => {
...
function ActivityPubKoa(options: { dispatch: IDispatch<IActivityPubEvent> }) {
...
function ActivityPubInboxKoa(options: {
  ...
  const koa = new Koa().use(async ctx => {
